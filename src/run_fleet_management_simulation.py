...
#logging system to record messages to log file and console
def setup_logging(log_file_path: str) -> logging.Logger:
    ...
    formatter = logging.Formatter('%(asctime)s - %(levelname)s - %(message)s') #Fromat of message
    console_handler.setFormatter(formatter)
    logging.getLogger('').addHandler(console_handler)
    logging.getLogger('matplotlib').setLevel(logging.WARNING)
    return logging.getLogger(__name__)

# Loads and manages the configuration data from the JSON files.
class ConfigManager:
    """
    Class handles loading and managing configurations.
    """
    # Defining attributes and assigning the information
    def __init__(self, config_paths: Dict[str, str]):
        self.config_data = self._load_json(config_paths['config']) #general config
        self.lif_data = self._load_json(config_paths['lif']) #graph
        self.agents_initialization_data = self._load_json(config_paths['agents_initialization']) #agents
        self.transportation_tasks_data = self._load_json(config_paths['transportation_tasks']) #tasks
    ...
    def _load_json(path: str) -> dict:
        ...
        """
        try:
            with open(path, 'r') as file: #read mode
                return json.load(file)
        except FileNotFoundError: #handling erros
...
def run_simulation(config_manager: ConfigManager, logging: logging.Logger):
    ...
    try:
        ...
        # Wait 10 s for the agent simulation to start.
        time.sleep(10)
    except subprocess.CalledProcessError as e:
        logging.error(f"Failed to execute {agent_simulation_path}: {e}")
        sys.exit(1)

    # Initialize the objects/components of the fleet manager.
